...
### FINAL 2 CHANGES (June 6, 2026) — Manager Files filters + Admin Holiday Calendar — verified
1. Manager Files filters: `FilesDashboard.js` hides the org-wide "All Managers" and "All Team Leads" dropdowns for Manager users (`{!isManager && (...)}`); GP filter stays (scoped via `/users/growth-partners`). Admin/Ops unchanged. Backend scope enforcement untouched — Manager still sees only their recursive subtree (verified 94 team files; manager-filter/tl-filter absent, gp-filter present).
2. Admin Holiday Calendar: NEW `utils/working_days.py` = single source of truth (`is_working_day`/`is_non_working`/`load_holiday_dates` cached 30s, `invalidate_holiday_cache`). NEW `holidays` collection + admin CRUD `GET/POST/PUT/DELETE /api/attendance/admin/holidays` (create/edit/delete admin-only; list HR/admin). NEW `HolidayCreate/HolidayUpdate` schemas. Holiday-aware working-day check now used by: Today's Attendance (`/admin/today`, `/team/today`), `/admin/summary` (absent + is_working_day), admin + my Monthly Matrix (WD/%, day marked "Holiday"), monthly & weekly summaries, WFH From→To expansion (attendance + leave_management paths), WFH approval/assign, and leave-day counting/attendance records. No hardcoded dates; historical attendance never modified (calendar only affects calculations). Admin-only UI added to the Attendance page ("Holidays" modal: add/edit/delete + list). Mobile unchanged (still 2.6.2/vc19).
...
### FINAL CLOSURE PASS (June 6, 2026) — verified
1. SATURDAY IS A WORKING DAY: `utils/working_days.py` weekly-off changed from Sat+Sun to SUNDAY ONLY (weekday==6). Holidays still override (incl. an explicitly-configured Saturday). Applied via the single shared helper to Today's Attendance, summary, matrix (label "Weekly Off" for Sunday), WD/%, P/L/W/A/U, WFH expansion/approval/assign, leave calcs. Matrix label checks in attendance.py updated Sat+Sun→Sun. Verified: June-2026 WD=26; Sat working, Sun "Weekly Off"; Sat-holiday override → WD 25.
2. MANAGER FILES GP FILTER: already functional on latest build (19 options / 18 GPs via scoped `/users/growth-partners`; files filter 94→20). No code change needed (earlier report was against the stale deployed build).
3. MANAGER DAILY TRACKING GP FILTER: same scoped `/users/growth-partners` source (18 GPs); component renders them. Verified endpoint scope.
4. TEAM LEAD GP FILTER (NEW): added a scoped GP dropdown to the TL web team pages (TeamData/TeamFiles/TeamCalls). Backend: `/leads?team_view` now honors `assigned_to` within the TL team; `/call-logs/team` gained `member_id`; `/files` already ANDs `gp_id` within team scope. Verified pinky (5 GPs): leads 14→6, files 20→10; out-of-team GP is leak-safe (files fail-closed to 0, leads fall back to team set — neither exposes another team).
5. INCOMING CALL ATTRIBUTION (CRITICAL): ROOT CAUSE — the GP dashboard's Incoming came from the mutable `daily_sessions.verified_incoming_calls` counter, written by multiple paths ($set on sync recompute + non-idempotent `$inc` on manual/mobile log endpoints) and it ignored the selected period; it was not recomputed strictly from the authenticated GP's own verified records per window, so it could show stale/duplicated/cross-context values. FIX (reports.py GP branch only): compute Incoming strictly from `verified_call_logs` filtered by the authenticated `user_id` (NEVER lead owner) within the selected period; naturally deduped by the sync insert guard (user_id+phone+device_timestamp) so re-syncs cannot inflate. Verified: Nithin=2 (own), teja personal=0 (isolated), period respected. Outgoing/post-call/sync-write/session $inc for outgoing untouched (070e552 + post-call modal + 0-sec guard preserved). Mobile stays 2.6.2/vc19.

*Last Updated: June 6, 2026*

### FINAL FAST FIX (June 6, 2026) — 3 items, verified (backend-only; no frontend/mobile change)
1. Manager/TL GP filters: Manager Files & Daily Tracking GP dropdowns already populate from scoped `/users/growth-partners` (Manager=18, verified 19 options incl. "All"; Files filter 94→20). TL web pages (TeamData/TeamFiles/TeamCalls) GP dropdown (added in prior pass) verified: pinky=5 GPs, leads 14→6, files 20→10, leak-safe. NO code change needed for item 1 on latest build — earlier "empty" was the stale deployed build.
2. Files Dashboard TWO independent date filters (same logic all roles): `utils/files_query.py` created clause now uses FILE-CREATED (`file_created_at`, fallback `created_at`) and the base no longer applies the `updated_at` activity clause. `get_files_dashboard_stats` now builds a date-independent authorized base, gates Total/New/In-Progress/by_status/loans_by_type by the file-created window, and keeps Approved/Disbursed on `stamp_in_range` (event dates). VERIFIED with a synthetic file (created Aug-2025, approved Sep-2025 ₹500000, disbursed Oct-2025 ₹480000): Aug-created→Total=1; Sep-activity→Approved=1/₹500000; Oct-activity→Disbursed=1/₹480000; Sep-created→Total=0. Independence proven (created window doesn't touch activity metrics and vice-versa). Role only narrows the authorized dataset. List regression OK (admin 513, manager 94).
3. Tracking Report Start/End: `reports.py daily-tracking-sheet` — Start Time = MIN(call_logs.created_at) that day (first ACTUAL call), End Time = MAX(attendance.check_out_time) that day; login/logout activity no longer used for these. "-" when no calls / no checkout. Manager access via existing recursive `resolve_report_scope` (verified: manager can load a team GP; out-of-team GP fail-closes). VERIFIED: first call 09:47 IST + checkout 18:30 IST → start 09:47, end 18:30. Call-based start is reassignment-safe (call_logs.user_id is immutable caller).
