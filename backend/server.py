...
async def migrate_form_filling_time(current_user: dict = Depends(require_admin)):
    ...
    For calls that don't have form_filling_seconds, we estimate based on:
    - If call duration > 60 seconds, estimate 30 seconds form filling
    - If call duration <= 60 seconds, estimate 15 seconds form filling
    """
    # Find all call_logs without form_filling_seconds
    ...
    for log in logs_without_form_time:
        ...
        # Estimate form filling time based on call duration
        if duration > 60:
            estimated_form_time = 30  # 30 seconds for longer calls
        else:
            estimated_form_time = 15  # 15 seconds for shorter calls
    ...
    for session in all_sessions:
        ...
        if user_id and session_date:
            # Calculate total form filling time from call_logs for this user on this date
...
async def get_daily_tracking_sheet(
    ...
    for tc in telecallers:
        ...
        while current_date < range_end:
            ...
            connected_count = len([c for c in day_calls if c.get("outcome") == "connected" or c.get("duration", 0) > 0])
