...
// Combined Totals (Connect + Meta) rendered ON TOP of the Connect tables.
export const CombinedTotalsCard = ({ connect, meta, testid = 'combined-totals' }) => {
  const c = connect || {};
  const m = meta || {};
  const calls = (c.total_calls || 0) + (m.total_calls || 0);
  const leads = (c.total_leads_generated || 0) + (m.total_leads_generated || 0);
  const file = (c.total_file || 0) + (m.total_file || 0);
  const talk = (c.total_call_seconds || 0) + (m.total_call_seconds || 0);
  ...
  return (
    <div className="card p-4 mb-6 border-l-4 border-indigo-500" data-testid={testid}>
      <div className="flex items-center gap-2 mb-4">
        ...
        <span className="text-xs text-gray-400">(Connect + Meta)</span>
      </div>
