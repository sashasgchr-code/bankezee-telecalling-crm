...
import { getLoanTypes } from '../services/api';

// SINGLE SOURCE OF TRUTH (client mirror of backend/config/loan_types.py).
// Served by GET /api/config/loan-types; this bundled list is the offline fallback and
// MUST stay identical to the backend canonical list so web/mobile/Meta never drift.
...
export const loanTypeLabel = (value, list = LOAN_TYPES) =>
...
// Hook: canonical list (backend-driven, bundled fallback) + helpers.
export function useLoanTypes() {
  const [loanTypes, setLoanTypes] = useState(LOAN_TYPES);
  const [vehicleValues, setVehicleValues] = useState(VEHICLE_LOAN_VALUES);
  useEffect(() => {
    let mounted = true;
    fetchLoanTypes().then((d) => {
      if (mounted && d && Array.isArray(d.loan_types) && d.loan_types.length) {
        setLoanTypes(d.loan_types);
        setVehicleValues(d.vehicle_loan_values || d.loan_types.filter((t) => t.vehicle).map((t) => t.value));
      }
    });
  ...
  return {
    loanTypes,
    vehicleValues,
    isVehicleLoan: (v) => vehicleValues.includes(v),
    label: (v) => loanTypeLabel(v, loanTypes),
  };
}
